...
class Deploy extends StatefulWidget {
  ...
  @override
  State<Deploy> createState() => _HomeScreenState();
}

class _HomeScreenState extends State<Deploy> {
  ...
  Future<void> download() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Stream<FileSystemEntity> entityStream = dir.list();
                        ...
                        child: OutlinedButton(
                          onPressed: () {
                            if (_couldDeploy){
                              ...
                              isDeploying = false;
                            } else if (_couldDeploy == false){
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('请先下载协议端！')),
                              );
                            } else if (isDeploying){
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('已经在部署中了！！')),
                              );
                            }
                          },
                          style: ButtonStyle(
